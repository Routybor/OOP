...
public class Deck {
    ...
    public Deck() {
        ...
        String[] ranks = {"Двойка", "Тройка", "Четверка", "Пятерка", "Шестерка", "Семерка", "Восьмерка", "Девятка", "Десятка", "Валет", "Королева", "Король", "Туз"};
        cards = new Card[52];
        int index = 0;
        int power;
        currentCardIndex = 0;
     ...
     *
     * @return следующая карта из колоды
     */
    public Card pickCard() {
        assert currentCardIndex < cards.length : "Некорректный индекс: колода пуста";
